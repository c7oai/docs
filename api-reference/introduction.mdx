...
<Note>
  Reporting scopes use `botconv` and `vocconv` resource names:
  `reporting:botconv` authorizes `GET /api/v1/reporting/tenants/{tenantId}/assistant/conversations`, and
  `reporting:vocconv` authorizes `GET /api/v1/reporting/tenants/{tenantId}/voc/conversations`.
</Note>
